...
from .models import Dept, Class, Student, Attendance, Subject, Teacher, Assign, AttendanceTotal, time_slots, DAYS_OF_WEEK, AssignTime, AttendanceClass, StudentCourse, Marks, MarksClass
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #auser = User.objects.create_user(username,raw_password)
            #auser.save()
...
def attendance_detail(request, stud_id, course_id):
    ...
    cr = get_object_or_404(Subject, id=course_id)
...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.save()
            sc.marks_set.create(type='I', name='Sessional 1')
            sc.marks_set.create(type='I', name='Sessional 2')
            sc.marks_set.create(type='I', name='Pre-University')
